...
def book():
    ...
    status = request.form['NeworUsed']
    version = request.form['Cover']
    if status =="---":
        status = "New"
    if version == "---":
        version = "Hardcover"
    ...
    return render_template("results.html", Valbook = vb, Amazon  = amz, ebay = eb, abebook = ab, Stat = status, cover = version, ValLink = valoreBooks, ALink = amazon,ELink = ebay, AbeLink=abebooks )
